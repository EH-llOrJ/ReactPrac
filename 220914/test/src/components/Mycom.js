...
// 일반 태그처럼 우리가 원하는 내용을 태그화 해서 사용할 수 있기 때문에 코드의 재활용성이 용이해진다.
// 이렇게 하면 좋은 점은 유지보수가 편하다.

// 리액트의 데이터 구조는 단방향(위에서 아래로 데이터를 전달)
// 매개변수로 받은 num의 명칭을 props이다.
// 부모 컴포넌트가 자식한테 데이터 전달할 수 있다.(자식이 줄 순 없다.) 객체 형태로 준다.
const Mycom = (num) => {
  const { name, age, Cname } = num;

  // 중괄호를 쓰는 이유는 자바스크립트 구문을 활용하겠다는 뜻
  return (
